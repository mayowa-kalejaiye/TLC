...
const contactReasons = [
  ...
  },
  {
    ...
    template: (name: string, message: string) => 
      `Hi, my name is ${name}. I'm a first timer at TLC. ${message}`,
  ...
  },
  {
    ...
    placeholder: 'Tell us why you want to join TLC...',
    template: (name: string, message: string) => 
      `Hi, my name is ${name}. I would like to become a member of The Light Community. ${message}`,
    description: 'Ready to make TLC your church home? Start your membership journey here.'
  },
  {
    id: 'counselling',
  ...
  },
  {
    ...
    template: (name: string, message: string) => 
      `Hi, my name is ${name}. I'm interested in partnering with TLC through giving. ${message}`,
  ...
  },
  {
    ...
    template: (name: string, message: string) => 
      `Hi, my name is ${name}. I would like to volunteer at TLC. ${message}`,
...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-tlc-cream pt-24 md:pt-32 pb-16">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-12">
          <div className="flex justify-center mb-6">
            <div className="bg-tlc-navy p-4 rounded-full">
              <MessageCircle className="h-12 w-12 text-tlc-gold" />
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 md:p-12">
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Reason Dropdown */}
            <div>
              <label className="block text-sm font-semibold text-tlc-navy mb-2 uppercase tracking-wider">
                Reason for Contact
              </label>
              <div className="relative">
                <button
                  ...
                  className="w-full bg-tlc-cream border-2 border-tlc-navy rounded-lg px-4 py-4 text-left flex items-center justify-between hover:border-tlc-gold transition-colors"
                >
                  <span className="font-semibold text-tlc-navy">{selectedReason.label}</span>
                  <ChevronDown className={`h-5 w-5 text-tlc-navy transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                
                {isDropdownOpen && (
                  <div className="absolute z-10 w-full mt-2 bg-white border-2 border-tlc-navy rounded-lg shadow-xl max-h-96 overflow-y-auto">
                    {contactReasons.map((reason) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-tlc-cream transition-colors border-b border-gray-200 last:border-b-0 ${
                          selectedReason.id === reason.id ? 'bg-tlc-cream' : ''
                        }`}
                      >
                        <div className="font-semibold text-tlc-navy">{reason.label}</div>
            ...
            {/* Name Input */}
            <div>
              <label htmlFor="name" className="block text-sm font-semibold text-tlc-navy mb-2 uppercase tracking-wider">
                Your Name
              </label>
              <input
                type="text"
                ...
                className="w-full bg-tlc-cream border-2 border-tlc-navy rounded-lg px-4 py-4 text-tlc-navy placeholder-gray-500 focus:outline-none focus:border-tlc-gold transition-colors"
                required
              />
            </div>
            ...
            {/* Message Input */}
            <div>
              <label htmlFor="message" className="block text-sm font-semibold text-tlc-navy mb-2 uppercase tracking-wider">
                Your Message
              </label>
              <textarea
                id="message"
                ...
                rows={6}
                className="w-full bg-tlc-cream border-2 border-tlc-navy rounded-lg px-4 py-4 text-tlc-navy placeholder-gray-500 focus:outline-none focus:border-tlc-gold transition-colors resize-none"
                required
              />
            </div>
            ...
            {/* Preview Section */}
            {name && message && (
              <div className="bg-tlc-cream border-2 border-gray-300 rounded-lg p-4">
            ...
            <button
              type="submit"
              className="w-full bg-tlc-navy text-white py-4 rounded-lg font-bold text-lg uppercase tracking-wider hover:bg-tlc-gold hover:text-tlc-navy transition-all duration-300 transform hover:scale-105 flex items-center justify-center space-x-2 shadow-lg"
            >
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-xl font-bold text-tlc-navy mb-4 uppercase">Service Times</h3>
          ...
          {/* Location */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-xl font-bold text-tlc-navy mb-4 uppercase">Physical Location</h3>
            <div className="space-y-3 text-gray-700">
              <div>
                <p className="font-semibold text-tlc-gold">The Light House</p>
                <p>43b Babaponmile Street</p>
                <p>After Winners Chapel</p>
                <p>Mangoro, Ikeja, Lagos</p>
              </div>
              <div className="mt-4">
                <p className="font-semibold text-tlc-gold">For Directions:</p>
                <p>Contact Isaac</p>
                <a href="tel:+2349058885465" className="text-tlc-navy hover:text-tlc-gold transition-colors">
                  0905 888 5465
                </a>
              </div>
            </div>
        ...
        {/* Call to Action */}
        <div className="mt-12 text-center bg-tlc-navy text-white rounded-lg p-8">
